...
const init = function (ws) {
    ...
    utils_1.add_event(ws, 'close', async (e) => {
        this.log('Closed.');
        this.open = false;
    ...
    utils_1.add_event(ws, 'message', (e) => {
        try {
...
const connectLib = function (ff) {
    ...
    this.ws = ws;
    utils_1.add_event(ws, 'error', utils_1.once((e) => {
        this.ws = null;
    ...
    if (config.socket && ws.readyState === 1) {
        ...
        utils_1.add_event(ws, 'open', utils_1.once((e) => {
            this.log('Opened.');
            init.call(this, ws);
            return ff(null);
        }));
